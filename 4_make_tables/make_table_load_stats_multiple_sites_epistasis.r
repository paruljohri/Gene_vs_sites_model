...
theta <- "theta0_005"
v_gammas <- c("gamma100_mean_lowrec") #keep this a single value: gamma100_mean_lowrec/ gamma100_mean
...
for (s_gamma in v_gammas) {
		...
		print (s_epsilon)
			for (s_dom in v_dom){
				...
				t_af_gene <- read.table(paste(s_folder, "/statistics/", num_sites, "/", theta, "/Epistasis/additive_gene_", s_gamma, "_", s_epsilon, "_", s_dom, ".af", sep=""), h=T)
				t_af_site <- read.table(paste(s_folder, "/statistics/", num_sites, "/", theta, "/Epistasis/additive_site_", s_gamma, "_", s_epsilon, "_", s_dom, ".af", sep=""), h=T)
				#genetic load:
				...
				v_data <- rbind(v_data, c(s_gamma, s_epsilon, s_dom, "allele_frequency_from_sample", "mean", mean(t_af_gene$mean_allele_freq), mean(t_af_site$mean_allele_freq)))
				v_data <- rbind(v_data, c(s_gamma, s_epsilon, s_dom, "allele_frequency_from_sample", "SE", sd(t_af_gene$mean_allele_freq)/sqrt(length(t_af_gene$mean_allele_freq)), sd(t_af_site$mean_allele_freq)/sqrt(length(t_af_site$mean_allele_freq))))
			}
		}
}
