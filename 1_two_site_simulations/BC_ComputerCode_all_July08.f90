...
!    These programs use iteration to calculate the equilibrium haplotype frequencies and load 
...
!   population genetic statistics for sites model
        if(e.le.0.001) then
        Rapp=0.0-0.5*q0*(1-q0)*(2*h*(1-2*h)*q0+(h**2)+(1-4*h*(1-h))*q0**2)*s
        Rapp=Rapp/(h+q0*(1-4*h))
        else
        Rapp=0-q0*(1-q0)*(h+(1-3*h)*q0)*e
        Rapp=Rapp/(h*(1-4*q0)+q0*(1+2*h*e))
        end if
    Dapp=Rapp*q0*(1-q0)
    write (1,*) ''
...
!   inbreeding load relative to additive value
    ...
    Rapp=0.5*q0*(1-q0)*(2*q0*(0.5-h-2*h*(1-2*h)*s)-(h**2)*s-2*(h+(1.5-2*h)*q0)*e)
