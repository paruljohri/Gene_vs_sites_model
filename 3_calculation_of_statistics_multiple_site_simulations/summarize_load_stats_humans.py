...
input_folder="/work/users/p/j/pjohri/FitnessNote/simulations/humans"
output_folder="/work/users/p/j/pjohri/FitnessNote/statistics/humans"
fitness_model="additive_gene" #additive_site/additive_gene
theta="theta0_00025"
l_gamma=["gamma850_mean"]
l_dom=["h_0_0", "h_0_2", "h_0_5"] #h_0_0/h_0_2/h_0_5

for gamma in l_gamma:
    for dom in l_dom:
        print (gamma + '\t' + dom)
        result = open(output_folder + "/" + theta + "/" + fitness_model + "_" + gamma + "_" + dom + ".txt", 'w+')
        ...
        while repnum <= 1000:
            ...
            f_out = open(input_folder + "/" + fitness_model + "/" + theta + "/" + gamma + "/" + dom + "/output" + str(repnum) + ".txt", 'r')
            for line in f_out:
                line1 = line.strip('\n')
                line2 = line1.split(": ")
