...
ListOfGamma=( "gamma100_mean" "gamma100_mean_lowrec") #gamma100_mean/gamma1000_mean/gamma100_mean_lowrec
	...
	do
		...
		do
			...
			echo "calculating mean allele frequency for " ${gamma} " and " ${dom}
                	python get_af_from_ms_general_reps.py -input_folder ${infolder}/${gamma}/${epsilon}/${dom} -output_folder ${outfolder} -extension ".ms" -output_prefix ${fitness_model}_${gamma}_${epsilon}_${dom} -num_indv 100 -num_sites ${num_sites}
		done
	done
done
