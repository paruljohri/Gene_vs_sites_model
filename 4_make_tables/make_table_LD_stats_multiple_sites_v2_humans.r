...
subfolder <- "humans"
theta <- "theta0_00025"
v_gammas <- c("gamma850_mean") #c("gamma2", "gamma20", "gamma100", "gamma2_mean", "gamma20_mean", "gamma100_mean", "gamma1000_mean")
v_dom <- c("h_0_0", "h_0_2", "h_0_5")
dist_min <- 1 #1
dist_max <- 100 #500/1000/2000
...
for (s_gamma in v_gammas) {
		...
		for (s_dom in v_dom){
			print (s_dom)
			t_add_gene <- read.table(paste(s_folder, "/statistics/", subfolder, "/", theta, "/additive_gene_", s_gamma, "_", s_dom, ".D", sep=""), h=T)
			t_add_site <- read.table(paste(s_folder, "/statistics/", subfolder, "/", theta, "/additive_site_", s_gamma, "_", s_dom, ".D", sep=""), h=T)
			...
			v_sigma1D_mean_gene <- t_D_reps_gene$v_D_gene/v_product_mean_gene
			v_sigma1D_mean_site <- t_D_reps_site$v_D_site/v_product_mean_site
			...
			v_data <- rbind(v_data, c(s_gamma, s_dom, "sigma1D_derived", "mean", mean(v_sigma1D_mean_gene), mean(v_sigma1D_mean_site)))
			v_data <- rbind(v_data, c(s_gamma, s_dom, "sigma1D_derived", "SE", sd(v_sigma1D_mean_gene)/sqrt(length(v_sigma1D_mean_gene)), sd(v_sigma1D_mean_site)/sqrt(length(v_sigma1D_mean_site))))
			...
			v_data <- rbind(v_data, c(s_gamma, s_dom, "sigma1D_minor", "mean", mean(v_sigma1D_mean_gene), mean(v_sigma1D_mean_site)))
			v_data <- rbind(v_data, c(s_gamma, s_dom, "sigma1D_minor", "SE", sd(v_sigma1D_mean_gene)/sqrt(length(v_sigma1D_mean_gene)), sd(v_sigma1D_mean_site)/sqrt(length(v_sigma1D_mean_site))))
		}
}

#name columns:
colnames(v_data) <- c("gamma", "dominance", "summary", "type", "additive_gene", "additive_site")
write.table(v_data, file = paste(s_folder, "/Tables/", subfolder, "/", theta, "/summary_LD_v2_humans_", dist_min, "_", dist_max, ".txt", sep=""), append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
