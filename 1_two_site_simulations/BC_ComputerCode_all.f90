...
!    statistics for the two-locus additive gene and sites models with no recombination, allowing for 
!     epistasis.

     program dom 4
!   recursions for 2-locus mutation model
    double precision :: h,u,x,y,z,p,q,q0,e,wx,wy,wbar,wz,s,x0,y0,disc,wprod
    ...
    write (1,*) 'Additive fitnesses plus epistasis'
...
10  write (*,*) 'Epistasis parameter?'
    read (*,*) e
    if(e.ge.1) go to 100
    write (1,*) ''
    write (1,*) 'Epistasis parameter= ',e
    write (*,*) 'Epistasis parameter= ',e
    x=x0
    y=y0
    z=1-2*x-y
    q=x+y
    ...
    igen=0
 50 if(ind.eq.0) then
    wx=1-(h+x+y+2*h*e*x+(1+h)*e*y)*s
    wy=1-2*(h+(1-h)*(x+y)+h*e+(1-h)*e*(x+y))*s
    wz=1-(x+y*(1+e))*2*h*s
    else
    wx=1-(h+2*(1-h)*x+(1.5-h)*y+1.5*e*y)*s
    wy=1-(2*(1+e)*(h+(1.5-2*h)*x+(1-h)*y))*s
    wz=1-(2*h*(x+(1+e)*y))*s
    end if
...
!   population genetic statistics for sites model
        if(e.le.0.001) then
        Rapp=0.0-0.5*q0*(1-q0)*(2*h*(1-2*h)*q0+(h**2)+(1-4*h*(1-4*h))*q0**2)*s
        Rapp=Rapp/(h*(1-4*q0)+q0)
        else
        Rapp=0-q0*(1-q0)*(h+(1-h)*q0)*e
        Rapp=Rapp/(h*(1-4*q0)+q0*(1+2*e))
        end if
    Dapp=Rapp*q0*(1-q0)
    write (1,*) ''
    ...
    write (1,*) ''
    write (1,*) ''
    write (1,*) 'Load statistics for sites model'
    write (1,*) ''
    ...
    Bs=Bs/(2*(dsqrt(u1)-u1))
       ...
       if(h.le.0.4999) then
       Bs=Bs/(4*u1*(1.0/(2*h)-1))
       else
       Bs=1-Bs
       end if
    end if
!   inbreeding load relative to additive value
    write (1,*) 'Relative L= ',Ls,' Relative B= ',Bs
    write (1,*) ''
    Vs=4*z*x*(h**2)+4*z*y*(h*(1+e))**2+8*(x*h*(1+e))**2+2*x**2
    Vs=Vs+4*x*y*((1+h)*(1+e))**2+(y*2*(1+e))**2-Ls0**2
    Vs=Vs/(4*u1)
    write (1,*) 'Relative V x h = ',Vs
    write (1,*) ''
    go to 10
    end if

    Rapp=0.5*q0*(1-q0)*(q0*(1-2*h)-(h**2)*s-2*(h+(0.75-h)*q0)*e)
...
!   population genetic statistics for sites model
    ...
    Rapp=0.5*q0*(1-q0)*(q0*((1-2*h)+(1-2*h*(2-3*h))*s))
    Rapp=Rapp/(h+2*(1-2*h)*q0)
    Dapp=Rapp*q0*(1-q0)
    write (1,*) ''
    write (1,*) 'Population genetic statistics for gene model'
    write (1,*) 'Gen= ',igen,' x= ',x1,'y= ',y1,' q= ',q
